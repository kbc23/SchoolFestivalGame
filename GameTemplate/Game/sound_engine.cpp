...
IXAudio2SourceVoice* SoundEngine::CreateXAudio2SourceVoice(WavFile* waveFile, bool is3DSound)
{
	if (!(waveFile->GetFormat()->nChannels <= INPUTCHANNELS)) {
		MessageBoxA(nullptr, "Channel over", "エラー。", MB_OK);
	}
	...
	if (is3DSound == false) {
		...
		{
			MessageBoxA(nullptr, "Failed CreateSourceVoice", "エラー。", MB_OK);
			return nullptr;
		}
	}
	else {
